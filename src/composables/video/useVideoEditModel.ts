...
export function useVideoEditModel(
  ...
  const { duration, frameMax, fps, width, height } = options

  const toTime = (frame: number) =>
    frameToTime(frame, duration.value, frameMax.value, fps.value)
  const toFrame = (time: number) =>
    timeToFrame(time, duration.value, frameMax.value, fps.value)
