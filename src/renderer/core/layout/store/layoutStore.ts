...
class LayoutStoreImpl implements LayoutStore {
  ...
  /**
   * Drops the geometry that belongs to the graph being left: slot, link and
   * reroute layouts, and the spatial indexes over them. Node entries are not
   * touched — those are registered and removed with the node itself, in
   * `LGraph.add` / `LGraph.remove`.
   */
  /**
